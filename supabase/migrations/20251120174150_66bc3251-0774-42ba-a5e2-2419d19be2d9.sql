...
-- Recreate the handle_new_user function without role switching
...
BEGIN
  ...
  INSERT INTO public.profiles (
    id, email, first_name, last_name, phone, user_type, username,
    city, district, favorite_team, interests, favorite_game_providers,
    company_name, company_tax_number, company_type, company_authorized_person,
    company_phone, company_email, company_address, company_website,
    company_description, contact_person_name, contact_teams, contact_telegram,
    contact_whatsapp, social_facebook, social_twitter, social_instagram,
    social_linkedin, social_youtube, social_telegram_channel, social_kick,
    social_discord, bio_link, support_email, social_pinterest
  )
  VALUES (
    new.id, new.email,
    ...
    CASE WHEN new.raw_user_meta_data->>'interests' IS NOT NULL 
         THEN string_to_array(new.raw_user_meta_data->>'interests', ',') ELSE NULL END,
    CASE WHEN new.raw_user_meta_data->>'favoriteGameProviders' IS NOT NULL 
         THEN string_to_array(new.raw_user_meta_data->>'favoriteGameProviders', ',') ELSE NULL END,
  ...
  VALUES (
    new.id,
    v_role,
    CASE WHEN v_account_type = 'site_owner' THEN 'pending'::user_status
         ELSE 'approved'::user_status END
  );

  -- Create initial loyalty points record - WITH EXPLICIT SCHEMA
