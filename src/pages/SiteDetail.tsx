...
import { BonusDetailModal } from '@/components/bonus/BonusDetailModal';
...
export default function SiteDetail() {
  ...
  const [selectedBonusCategory, setSelectedBonusCategory] = useState('all');
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-background to-muted pt-[72px] md:pt-[84px]">
            ...
            {bonusOffers && bonusOffers.length > 0 ? (
              <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {['all', 'casino', 'spor'].map(cat => (
                    <Button
                      key={cat}
                      variant={selectedBonusCategory === cat ? 'default' : 'outline'}
                      onClick={() => setSelectedBonusCategory(cat)}
                      size="sm"
                    >
                      {cat === 'all' && 'TÜM'}
                      {cat === 'casino' && 'CASINO'}
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  {bonusOffers
                    .filter((bonus: any) => 
                      selectedBonusCategory === 'all' || 
                      (bonus as any).category === selectedBonusCategory
                    )
                    .map((bonus: any) => (
                      <BonusCard
                        key={bonus.id}
                        ...
                        onDetailsClick={() => setSelectedBonus(bonus)}
                        onClaimClick={handleAffiliateClick}
                      />
                    ))}
                </div>
      ...
      {selectedBonus && (
        <BonusDetailModal
          ...
          bonus={selectedBonus}
          siteName={site?.name}
          affiliateLink={site?.affiliate_link}
        />
      )}
